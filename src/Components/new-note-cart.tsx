...
export function NewNoteCart({ onNoteCreated }: newNoteCartProps) {
  ...
  function handleContentChanged(event: ChangeEvent<HTMLTextAreaElement>) {
    ...
    if (event.target.value == "") setSouldShowOnBoarding(true);
  }
  ...
  function handleStartRecording() {
    ...
    if (!isSpeechRecognitionAPIAvailable) {
      alert("Infelizmente seu navegador não suporta a API");
      setIsRecording(false);
      return;
    }
    setIsRecording(true);
    setSouldShowOnBoarding(false);
    const RegognitionAPi = window.SpeechRecognition || window.SpeechRecognition;
    speechRecognition = new RegognitionAPi();
          ...
          <form className="flex-1 flex flex-col">
            ...
            {isRecording ? (
              <button
                ...
                className="w-full bg-slate-900 py-4 text-center text-sm text-slate-300 font-medium hover:text-slate-100 flex items-center justify-center gap-2"
              >
